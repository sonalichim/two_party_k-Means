...
PaillierSystem::PaillierSystem (  int x)
{
    K = 2*x;
    primeSize=x;
...
PaillierSystem::PaillierSystem(  int x, mpz_t pk)
{
    K =2*x;
    primeSize=x;
...
PaillierSystem::PaillierSystem(  int x, mpz_t pk, mpz_t sk)
{
    K =2*x;
    primeSize=x;
...
void PaillierSystem::addCiphers(mpz_t sumCiph, mpz_t c1, mpz_t c2)
{
    ...
    mpz_mod(sumCiph, sumCiph, Msqr);
}

void PaillierSystem::getThreeFactorPaillier(  int * newSize, mpz_t newM, PaillierSystem **pObj)
{
      int m, i=3;
    mpz_t gcd;
    mpz_init(gcd);
    ...
    *newSize = K + i;
    *newSize =  (*newSize) /2;
    *pObj = new PaillierSystem(*newSize, newM, newPhi);
    *newSize = (*newSize) *2;
